...
Reader = module.exports = function () {
    this._cachedIpv4StartNode = false;
};
...
Reader.prototype.getSync = function getSync(ipAddress, path) {
    var pointer = this.findAddressInTree(ipAddress);
    return (pointer === 0) ? null : this.resolveDataPointerSync(pointer, path);
};

Reader.prototype.get = function get(ipAddress, path, callback) {
    ...
    if(pointer === 0){
        ...
        this.resolveDataPointer(pointer, path, callback);
    };
};
...
Reader.prototype.findAddressInTree = function findAddressInTree(ipAddress) {
    ...
    if (this._cachedIpv4StartNode !== false) {
        i = 96;
        nodeNum = this._cachedIpv4StartNode;
    }
    ...
    for (i; i < len; i++) {
        ...
        if (i >= ipStartBit) {
            if (isIp4AddressInIp6Db && this._cachedIpv4StartNode === false) {
                this._cachedIpv4StartNode = nodeNum;
            }
...
Reader.prototype.resolveDataPointerSync = function resolveDataPointerSync(pointer, path) {
    var resolved = pointer - this.metadata.getNodeCount() + this.metadata.getSearchTreeSize();

    return this.decoder.decodeSync(resolved, path)[0];
};

Reader.prototype.resolveDataPointer = function resolveDataPointer(pointer, path, callback) {
    var resolved = pointer - this.metadata.getNodeCount() + this.metadata.getSearchTreeSize();
